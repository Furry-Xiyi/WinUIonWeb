<template>
  ...
  <span style="font-size: 14px; font-weight: 600; margin-bottom: 6px;">{{ $t('text.appearance') }}</span>
  <WinExpander>
    <template #header>
      <div style="display: flex; align-items: center; gap: 12px;">
        <span class="icon" style="font-size: 20px; width: 25px; text-align: center;"></span>
        <div style="display: flex; flex-direction: column;">
          <span style="font-size: 14px;">{{ $t('text.theme') }}</span>
          <span style="font-size: 12px; color: var(--text-secondary); margin-top: 2px;">{{ $t('text.choose-your-app-color-mode') }}</span>
        </div>
      </div>
    </template>
