<template>
  <div class="win-calendar-view">
    <div class="calendar-header">
      <button class="win-btn subtle calendar-title-btn"
              @click="onLabelClick"
              :disabled="viewMode === 2">
        {{ labelText }}
      </button>
    ...
    <div class="calendar-view-body">
      <Transition :css="false" @before-leave="onBeforeLeave" @leave="onLeave" @before-enter="onBeforeEnter" @enter="onEnter">
        <!-- 日视图 (7列) -->
        <div v-if="viewMode === 0" key="day" class="calendar-panel">
          ...
          <div class="calendar-scroll" ref="dayScrollEl" @scroll="onDayScroll">
            <div :style="{ height: dayTotalHeight + 'px', position: 'relative' }">
              <div :style="{ position: 'absolute', top: dayRenderTop + 'px', left: 0, right: 0 }">
                <div class="calendar-grid">
                  <button v-for="cell in dayRenderCells"
                          :key="cell.key"
                          class="calendar-day"
                          :class="{
                    ...
                    <span v-if="cell.showLabel && isGroupLabelVisible"
                          class="group-label"
        ...
        <div v-else-if="viewMode === 1" key="month" class="calendar-panel">
          <div class="calendar-scroll large-scroll" ref="monthScrollEl" @scroll="onMonthScroll">
            <div :style="{ height: monthTotalHeight + 'px', position: 'relative' }">
              <div :style="{ position: 'absolute', top: monthRenderTop + 'px', left: 0, right: 0 }">
                <div class="calendar-large-grid">
                  <button v-for="item in monthRenderItems"
                          :key="item.key"
                    ...
                    <span v-if="item.showLabel && isGroupLabelVisible"
                          class="group-label"
        ...
        <div v-else key="year" class="calendar-panel">
          <div class="calendar-scroll large-scroll" ref="yearScrollEl" @scroll="onYearScroll">
            <div :style="{ height: yearTotalHeight + 'px', position: 'relative' }">
              <div :style="{ position: 'absolute', top: yearRenderTop + 'px', left: 0, right: 0 }">
                <div class="calendar-large-grid">
                  <button v-for="item in yearRenderItems"
                          :key="item.key"
                          class="calendar-large-btn"
                          :class="{
                            'out-of-scope': item.outOfScope,
...
const shortMonths = computed(() => props.language === 'English'
  ? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
  : ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月']
);
const monthNames = computed(() => props.language === 'English'
  ? ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
  : ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']
);
const dayNames = computed(() => props.language === 'English' ? ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'] : ['日', '一', '二', '三', '四', '五', '六']);
...
const monthMeta = (() => {
  ...
  for (let y = MIN_YEAR; y <= MAX_YEAR; y++) {
    for (let m = 0; m < 12; m++) {
      ...
      arr.push({ y, m, startRow: accRow, rows, firstDow: first, dim });
      accRow += rows;
    }
  }
  return { data: arr, totalRows: accRow };
})();

const findMonthByRow = (row) => {
  let lo = 0, hi = monthMeta.data.length - 1;
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    if (monthMeta.data[mid].startRow <= row) lo = mid; else hi = mid - 1;
  }
  return lo;
};
...
const computeDayView = () => {
  ...
  const visibleFirstRow = Math.floor(visibleTopPx / ROW_H);
  const visibleLastRow = Math.floor((visibleBotPx - 1) / ROW_H);
  const totalVisiblePx = scrollH;
  const threshold = totalVisiblePx * 0.75;

  let bestIdx = findMonthByRow(visibleFirstRow);
  let bestPx = 0;
  let r = visibleFirstRow;
  while (r <= visibleLastRow) {
    const mi = findMonthByRow(r);
    const mm = monthMeta.data[mi];
    const monthEndRow = mm.startRow + mm.rows - 1;
    const segFirstRow = r;
    const segLastRow = Math.min(monthEndRow, visibleLastRow);
    const segTopPx = Math.max(segFirstRow * ROW_H, visibleTopPx);
    const segBotPx = Math.min((segLastRow + 1) * ROW_H, visibleBotPx);
    const px = segBotPx - segTopPx;
    if (px > bestPx) { bestPx = px; bestIdx = mi; }
    r = segLastRow + 1;
  }

  if (bestPx >= threshold) {
    headerMonth.value = monthMeta.data[bestIdx].m;
    headerYear.value = monthMeta.data[bestIdx].y;
  } else {
    headerMonth.value = monthMeta.data[bestIdx].m;
    headerYear.value = monthMeta.data[bestIdx].y;
  }
  ...
  for (let row = startRow; row < endRow; row++) {
    const mi = findMonthByRow(row);
    const mm = monthMeta.data[mi];
    const localRow = row - mm.startRow;
    for (let c = 0; c < 7; c++) {
      const i = localRow * 7 + c;
      const dayNum = i - mm.firstDow + 1;
      let cy = mm.y, cm = mm.m, cd = dayNum;
      if (dayNum < 1) {
        const pd = new Date(mm.y, mm.m, 0);
        cy = pd.getFullYear(); cm = pd.getMonth(); cd = pd.getDate() + dayNum;
      } else if (dayNum > mm.dim) {
        cd = dayNum - mm.dim; cm = mm.m + 1;
        if (cm > 11) { cm = 0; cy++; }
      }
      const fullDate = new Date(cy, cm, cd);
      cells.push({
        key: `${row}-${c}`,
        date: cd, month: cm, year: cy,
        outOfScope: cy !== scopeYear || cm !== scopeMonth,
        isToday: fullDate.toDateString() === todayStr,
        showLabel: cd === 1 && c === 0 || (cd === 1 && i === mm.firstDow),
...
const monthTotalHeight = computed(() => totalMonthPages * LARGE_PAGE_H);
...
const computeMonthView = () => {
  ...
  const threshold = viewH * 0.75;

  const startRow = Math.max(0, Math.floor(st / LARGE_ROW_H) - 1);
  const endRow = Math.min(totalMonthPages * LARGE_PAGE_ROWS, Math.ceil(visibleBotPx / LARGE_ROW_H) + 1);
  ...
  let r = visibleFirstRow;
  while (r <= visibleLastRow) {
    const yr = MIN_YEAR + Math.floor(r / LARGE_PAGE_ROWS);
    const yearEndRow = (yr - MIN_YEAR + 1) * LARGE_PAGE_ROWS - 1;
    ...
    const px = segBotPx - segTopPx;
    if (px > bestPx) { bestPx = px; bestYear = yr; }
    r = segLastRow + 1;
  }
  ...
  const items = [];
  for (let row = startRow; row < endRow; row++) {
    const pageIdx = Math.floor(row / LARGE_PAGE_ROWS);
    const localRow = row - pageIdx * LARGE_PAGE_ROWS;
    ...
    for (let c = 0; c < 4; c++) {
      ...
      if (m < 12) {
        items.push({
          key: `m${row}-${c}`,
          month: m, year: yr, text: shortMonths.value[m],
        ...
        items.push({
          key: `m${row}-${c}`,
          month: nextM, year: nextYr, text: shortMonths.value[nextM],
          outOfScope: true,
          isTodayMonth: nextM === todayMonth && nextYr === todayYear,
          showLabel: nextM === 0 && c === 0,
          labelText: `${nextYr}`
        });
      }
    }
...
const DECADE_GRID = 16;
const DECADE_PAGE_ROWS = 4;

const yearScrollTop = ref(0);
const yearRenderTop = ref(0);
const yearRenderItems = ref([]);
const totalDecadePages = Math.ceil((MAX_YEAR - MIN_YEAR + 1) / DECADE_SIZE);
const yearTotalHeight = computed(() => totalDecadePages * LARGE_PAGE_H);
...
const computeYearView = () => {
  ...
  const startRow = Math.max(0, Math.floor(st / LARGE_ROW_H) - 1);
  const endRow = Math.min(totalDecadePages * DECADE_PAGE_ROWS, Math.ceil(visibleBotPx / LARGE_ROW_H) + 1);
  yearRenderTop.value = startRow * LARGE_ROW_H;

  const visibleFirstRow = Math.floor(visibleTopPx / LARGE_ROW_H);
  const visibleLastRow = Math.floor((visibleBotPx - 1) / LARGE_ROW_H);

  let bestDecadeStart = MIN_YEAR;
  let bestPx = 0;
  let r = visibleFirstRow;
  while (r <= visibleLastRow) {
    const pageIdx = Math.floor(r / DECADE_PAGE_ROWS);
    const decadeStart = MIN_YEAR + pageIdx * DECADE_SIZE;
    const pageEndRow = (pageIdx + 1) * DECADE_PAGE_ROWS - 1;
    const segLastRow = Math.min(pageEndRow, visibleLastRow);
    const segTopPx = Math.max(r * LARGE_ROW_H, visibleTopPx);
    const segBotPx = Math.min((segLastRow + 1) * LARGE_ROW_H, visibleBotPx);
    const px = segBotPx - segTopPx;
    if (px > bestPx) { bestPx = px; bestDecadeStart = decadeStart; }
    r = segLastRow + 1;
  }
  ...
  const items = [];
  for (let row = startRow; row < endRow; row++) {
    const pageIdx = Math.floor(row / DECADE_PAGE_ROWS);
    const localRow = row - pageIdx * DECADE_PAGE_ROWS;
    const decadeStart = MIN_YEAR + pageIdx * DECADE_SIZE;
    for (let c = 0; c < 4; c++) {
      const idx = localRow * 4 + c;
...
const isSelected = (cell) => {
  ...
  return Array.isArray(props.modelValue) && props.modelValue.some(v => v.toDateString() === d.toDateString());
};
...
const onDayScroll = () => { if (dayScrollEl.value) { dayScrollTop.value = dayScrollEl.value.scrollTop; computeDayView(); } };
const onMonthScroll = () => { if (monthScrollEl.value) { monthScrollTop.value = monthScrollEl.value.scrollTop; computeMonthView(); } };
const onYearScroll = () => { if (yearScrollEl.value) { yearScrollTop.value = yearScrollEl.value.scrollTop; computeYearView(); } };
...
const scrollDayTo = (y, m, smooth = false) => {
  ...
  nextTick(() => {
    ...
    else { dayScrollEl.value.scrollTop = top; dayScrollTop.value = top; computeDayView(); }
  });
};

const scrollMonthTo = (y, smooth = false) => {
  const pageIdx = y - MIN_YEAR;
  const top = pageIdx * LARGE_PAGE_H;
  nextTick(() => {
    if (!monthScrollEl.value) return;
    if (smooth) monthScrollEl.value.scrollTo({ top, behavior: 'smooth' });
    else { monthScrollEl.value.scrollTop = top; monthScrollTop.value = top; computeMonthView(); }
  });
};

const scrollYearTo = (dec, smooth = false) => {
  const pageIdx = Math.floor((dec - MIN_YEAR) / DECADE_SIZE);
  const top = pageIdx * LARGE_PAGE_H;
  nextTick(() => {
    if (!yearScrollEl.value) return;
    if (smooth) yearScrollEl.value.scrollTo({ top, behavior: 'smooth' });
    else { yearScrollEl.value.scrollTop = top; yearScrollTop.value = top; computeYearView(); }
  });
};

const onNav = (dir) => {
  if (viewMode.value === 0) {
    let m = headerMonth.value + dir, y = headerYear.value;
...
const onLabelClick = () => {
  if (viewMode.value === 0) { transitionDir.value = 'out'; viewMode.value = 1; nextTick(() => scrollMonthTo(headerYear.value)); }
  else if (viewMode.value === 1) { transitionDir.value = 'out'; viewMode.value = 2; nextTick(() => scrollYearTo(headerDecade.value)); }
};
...
const onSelectDay = (cell) => {
  ...
  if (props.selectionMode === 'Single') emit('update:modelValue', cell.fullDate);
    ...
    if (idx >= 0) list.splice(idx, 1); else list.push(cell.fullDate);
    emit('update:modelValue', list);
  }
};

const onBeforeEnter = (el) => { el.style.position = 'absolute'; el.style.inset = '0'; };
const onEnter = (el, done) => {
  const cls = transitionDir.value === 'out' ? 'page-transition-fade-reverse' : 'page-transition-fade';
...
const onBeforeLeave = (el) => { el.style.position = 'absolute'; el.style.inset = '0'; };
const onLeave = (el, done) => {
  const cls = transitionDir.value === 'out' ? 'page-transition-fade' : 'page-transition-fade-reverse';
  el.style.animationDirection = 'reverse'; el.classList.add(cls);
  el.addEventListener('animationend', () => { el.classList.remove(cls); el.style.animationDirection = ''; done(); }, { once: true });
};
